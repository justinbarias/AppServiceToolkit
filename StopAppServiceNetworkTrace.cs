...
{
    public static class StopAppServiceNetworkTrace
    {
        [FunctionName("StopAppServiceNetworkTrace")]
        public static IActionResult Run([HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = null)]HttpRequest req, ILogger log, ExecutionContext context)
        {
            ...
            try {
                ...
                log.LogInformation("Stopping network trace!");
                AzureWebAppService service = new AzureWebAppService(azure, log, null);
                var res = service.StopNetworkTrace(resourceGroupName, webAppName);
                Task.WaitAll(res);
                result = res.Result;
            }
            catch(Exception e)
